...
LRESULT WINAPI MainWindowProcc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	case MESSAGE_STARTGAME_IN:
	{
		...
		int res = MessageBox(hwnd, Connection->PlayerName, L"ѕрин€ть приглошение?", MB_YESNO | MB_ICONINFORMATION);
		if (res == IDYES)
		{
			PplayGround Play = GetWindowLong(hwnd, 0);
			Play->YouStep = 0;
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR lpCmdLine, INT nCmdShow)
{
	...
	HWND HBStart = CreateWindowEx(0, L"BUTTON", L"—писок игроков", (WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON), MINIMAL_WIDTH_WINDOW - 140, 0, 120, 30, Mainhwnd, (HMENU)MAIN_WINDOW_BUTTON_START, 0, NULL);
